...
function payFarmer(id) {
  ...
  const b = bookings.find((x) => x.id === id);
  if (!b) return;
  const amount = Number(prompt(`${t("amount")}`, String((Number(b.qty) || 1) * 2200))) || 0;
  if (CFG.RAZORPAY_KEY && window.Razorpay) {
    const rzp = new window.Razorpay({
      key: CFG.RAZORPAY_KEY,
      amount: Math.round(amount * 100),
      currency: "INR",
      name: "KisanQ",
      description: `Pay ${b.name} · ${b.token}`,
      handler() {
        finishPay(id);
      }
    });
  ...
  document.getElementById("payModalBody").innerHTML = `
    ...
    <p>₹ ${amount}</p>
    <p class="sub">${t("payDemo")}</p>
    <button class="primary" onclick="finishPay('${id}')">${t("confirmPay")}</button>
